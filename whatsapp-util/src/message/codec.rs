...
use crate::message::{tag, tokens, BinaryMessage};
use crate::model::contact_jid::{ContactJid, Server};
...
impl NodeCodec {
    pub fn new(decoded: &[u8], session: &Session) -> Result<Self> {
        let decrypted = aes::decrypt_no_hash(
            session.read_key,
            aes::iv_as_nonce(session.read_cnt),
            decoded,
        )?;

        Ok(Self {
            buffer: ByteBuffer::from_bytes(&decrypted),
        })
    }
    ...
    fn read_string_from_token(&mut self, token: i32) -> &'static str {
        if token < tag::DICTIONARY_ZERO || token > tag::DICTIONARY_THREE {
            return tokens::SINGLE_BYTE[(token - 1) as usize];
        }

        let delta = (tokens::DOUBLE_BYTE.len() / 4) as i32 * (token - tag::DICTIONARY_ZERO);
        tokens::DOUBLE_BYTE[(self.buffer.read_u8() as i32 + delta) as usize]
    }
    ...
    fn read_string_mode(&mut self, mut output: String, hex: bool) -> String {
        ...
        loop {
            ...
            let token = self.buffer.read_u8() as u32;
            if hex {
                output.insert(index, tokens::HEX[(token >> 4) as usize]);
                output.insert(index + 1, tokens::HEX[(15 & token) as usize]);
            } else {
                output.insert(index, tokens::NUMBERS[(token >> 4) as usize]);
                output.insert(index + 1, tokens::NUMBERS[(15 & token) as usize]);
            }
